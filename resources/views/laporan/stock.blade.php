...
<div style="margin-buttom : 15px;">
    <table width="100%" border="0">
        <tr>
            <td colspan="2" align="center" style="font-size:18px;"> <strong> LAPORAN STOCK </strong> </td>
        </tr>
        <tr>
...
<table id="TabelKonten" border="1" style="border-collapse: collapse; border-color: #000; margin-bottom : 10px;"
    ...
    <thead class="text-center">
        <tr>
            ...
            <th class="align-middle">Actual Value</th>
            <th class="align-middle">Unit</th>
            <th class="align-middle">Content</th>
            <th class="align-middle">Actual Last Price</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-right">{{number_format($r->stock*$r->harga_jual,0,',','.')}}</td>
            <td class="text-center">{{$r->nama_satuan}}</td>
            <td class="text-center">1</td>
            <td class="text-right">{{number_format($r->last_purchase_price,0,',','.')}}</td>
            <td class="text-right">{{number_format($r->harga_jual,0,',','.')}}</td>
            <td class="text-center">{{ $r->jenis===1?'Barang Baku' : 'Barang Jadi' }}</td>
        </tr>
        @php
        $i++;
        $total+=$r->stock*$r->harga_jual;
        @endphp
        @endforeach
        <tr>
            <td></td>
