  ...
  <form @submit.prevent="saveSale()" autocomplete="off">
    <div class="card">
      <div class="card-header">
        <h4 class="card-title">Ubah Penjualan</h4>
...
export default {
  ...
  methods: {
    getEdit() {
      axios
        .get(`${this.$baseUrl}api/penjualan/${this.$route.params.id}`)
        .then((response) => {
          ...
          datafind.detail.forEach((value) => {
            this.form.barangbuy.push({
              ...
              price: value.harga,
              priceActual: value.actual_harga,
    ...
    saveSale() {
      ...
      this.disabled = true;
      this.form
        .put(`${this.$baseUrl}api/penjualan/${this.form.notrx}`)
        .then((response) => {
          Toast.fire({
