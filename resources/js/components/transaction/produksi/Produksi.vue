          ...
          <table class="table table-bordered nowrap">
            ...
            <tbody>
              <tr v-for="(r, i) in paginatedDatas" :key="i">
                ...
                <td class="text-end">
                  {{ vueNumberFormat(r.sell_price, { precision: 0 }) }}
                </td>
...
export default {
  ...
  methods: {
    ...
    formTambah() {
      this.$router.push({ name: "produksi.add" });
    },
    formUpdate(item) {
      this.$router.push({
        name: "produksi.edit",
    ...
    getData() {
      axios
        .get(`${this.$baseUrl}api/produksi`, { params: this.tableShow })
        .then((response) => {
      ...
      if (this.search) {
        datas = datas.filter((row) => {
          return Object.keys(row).some((key) => {
            if (key.toLowerCase() === "id" || key.toLowerCase() === "sell_price") {
              return (
