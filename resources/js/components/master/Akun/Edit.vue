...
export default {
  ...
  created() {
    ...
    if (!this.edit) {
      ...
      const kodesub = this.edit.kode_sub.split("-");
      this.form.id = this.edit.id;
      this.form.kode_sub = kodesub[0];
      this.form.desc = this.edit.nama_akun;
      this.form.akuntype = this.edit.type;
    }
  ...
  methods: {
    ...
    saveAkun() {
      ...
      this.disabled = true;
      this.form
        .put(`${this.$baseUrl}api/akun/${this.form.id}`)
        .then((respon) => {
          Toast.fire({
