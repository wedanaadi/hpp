            ...
            <tbody>
              <tr v-for="(r,i) in filteredDatas" :key="i">
                ...
                <td class="text-center">
                  <span v-if="r.hak_akses === 1" class="badge bg-info text-dark">STORE KEEPER</span>
                  <span v-else-if="r.hak_akses === 2" class="badge bg-info text-dark">KEUANGAN</span>
                  <span v-else-if="r.hak_akses === 3" class="badge bg-info text-dark">F&B SERVICE</span>
                  <span v-else class="badge bg-info text-dark">F&B PRODUCT</span>
                </td>
                <td class="text-center">
        ...
        <div>
          ...
          <nav class="pagination" v-else>
            ...
            <a
              ...
              @click="++pagination.currentPage"
            >Next</a>
            <button class="btn btn-sm pagination-next btn-secondary" v-else disabled>Next</button>
          </nav>
        </div>
        <!-- end pagination -->
      </div>
        ...
        <form @submit.prevent="isSave ? simpanData() : ubahData()">
            ...
            <div class="form-group">
              <div class="form-group">
                ...
                <v-select
                  ...
                  :class="{ 'select-invalid' : form.errors.has('hak_akses') }"
                ></v-select>
          ...
          <div class="modal-footer">
            <button type="submit" class="btn btn-primary" :disabled="disabled">
              <div v-show="loading" class="spinner-border text-white" role="status"></div>Simpan
            </button>
          </div>
        </form>
      </div>
...
export default {
  data() {
    ...
    return {
      ...
      datas: [],
      optHK: [
        { code: "1", label: "STORE KEEPER" },
        { code: "2", label: "KEUANGAN" },
        { code: "3", label: "F&B SERVICE" },
        { code: "4", label: "F&B PRODUCT" },
      ],
      form: new Form({
        id: "",
        nama: "",
  ...
  methods: {
    getData() {
      axios
        .get(`${this.$baseUrl}api/user`, { params: this.tableShow })
        .then((response) => {
    ...
    ubahData() {
      ...
      this.disabled = true;
      this.form
        .put(`${this.$baseUrl}api/user/${this.form.id}`)
        .then((respon) => {
          this.getData();
          $("#modal").modal("hide");
          Toast.fire({
